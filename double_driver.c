...
static ssize_t device_write(struct file *file, const char __user *user_buffer, size_t size, loff_t *offset) {
   ...
   /* for(i=size-1,j=1;i>=0;i--,j=j*10)
	    sum=((device_buffer[i]-48)*j)+sum;    */
	  
    for(i=0;i<size-1;i++)
	    sum=(sum*10)+(device_buffer[i]-48);
	 
    printk(KERN_INFO "simple_device: sum= %du bytes from the user\n", sum);
    sum=sum*2;

    printk(KERN_INFO "simple_device: sum= %du bytes from the user\n", sum);
    for(temp=sum;temp;count++,temp/=10);

    for(i=count-1;sum;sum=sum/10,i--)
	    device_buffer[i]=(sum%10)+48;
